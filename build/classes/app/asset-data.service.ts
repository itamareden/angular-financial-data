  ...
  constructor(private http : Http) { }
    ...
    getAssetsDataForPerformance(assets:Asset[]): Observable<AssetInMarketMoversTable[]> {
       ...
       let symbols;
        ...
        return this.http.get(this.urlForPresentAssetData+'&symbols='+symbols)
...
function  toCandlestick(result:any): Candlestick{
  ...
  let candleStick = <any>({
    ...
    dayOfTheMonth: result.tradingDay.substring(8,10),  // for calculating asset returns   
    open: result.open,
    low: result.low,
    high: result.high,
    close: result.close,
...
function mapAssetsDataForPerformance(res: Response): AssetInMarketMoversTable[] {
    ...
    let assetsDataForPerformance = result.map(asset=> {
        ...
        let assetDataForPerformance = <any> ({
                                            symbol:asset.symbol,
                                            netChangePercent: asset.percentChange,
