  ...
  constructor(private utils: UtilsService) { }
    ...
    createBarChart(dataSeries:any[], chartProperties:any, prop:string) :any{
        ...
        if(this.checkChartProperties(dataSeries,chartProperties)){
            ...
            chartProperties.xAxisTop = this.calculateXAxisTop(chartProperties,highest,lowest);
    ...
    calculateXAxisTop(chartProperties: any, highest: number, lowest: number):number{
    ...
    calculateInternalAxis(chartProperties: any, highest: number, lowest: number){
        ...
        console.log(chartProperties.roundTarget*4)
        console.log(chartRange/(chartProperties.roundTarget*4))
        if(this.utils.calculateModuloSafely(chartRange,(chartProperties.roundTarget*5))==0 || this.utils.calculateModuloSafely(chartRange,(chartProperties.roundTarget*4))==0){ // for example: 2.4%(0.1*4)=0
            if(this.utils.calculateModuloSafely(chartRange,(chartProperties.roundTarget*5))==0){console.log('inside 5..')
                interval=(chartRange/5);
                numOfAxis=5;
            }else{console.log('inside 4..')
...
//            numOfAxis=5;    
        ...
        for(let i=0; i<numOfAxis; i++){
            let internalAxisProp={value:null, top:null};
            internalAxis+=interval;
            internalAxisProp.value = this.roundNumberToSpecificDecimalPlaces(internalAxis, chartProperties.DADP);
            internalAxisProp.top = chartProperties.topMargin+chartProperties.totalBarsHeight*(1-internalAxisProp.value/chartRange);
