...
export class AssetFilterPipe implements PipeTransform {
    ...
    transform(assets:Asset[], searchedWord: string,isShowList:boolean,prior:string) {
        
        if(prior!=undefined) searchedWord=prior;
        if(isShowList==false || searchedWord==null || searchedWord.length==0) return
